...
void main()
{
//    time = time + clamp(10.0 * lowFreq, 0.0, 0.75);
    float a = 200.0 * lowFreq * lowFreq + 0.1;
    vec2 p = -1.0 + 2.0 * gl_FragCoord.xy / resolution.xy;
    gl_FragColor= a * vec4(s(vec3(sin(time*1.5)*.5,cos(time)*.5,time), normalize(vec3(p.xy,1.0))),1.0);
}
